...
'''
    Clase que implementa el algoritmo de SARSA para la resolución de problemas de aprendizaje por refuerzo.
            Q[estado][accion] += alpha * (recompensa + gamma *Q[siguiente_estado][siguiente_accion] - Q[estado][accion])
...
class Sarsa:
    ...
    ################################################### MÉTODOS ######################################################################
        ...
        entrenar(destino,mapa): 
            ...
            Entrena el algoritmo de SARSA y devuelve la función Q aprendida. Con un criterio de parada basado en el número de iteraciones y numero de pasos a realizar.
    ...
    # explorativo: Trata de descubrir nuevas opciones, sin priorizar la obtención de recompensas inmediatas.
# explotativo: Aprovecha al máximo el conocimiento adquirido hasta el momento para obtener las mayores recompensas o resultados esperados.
